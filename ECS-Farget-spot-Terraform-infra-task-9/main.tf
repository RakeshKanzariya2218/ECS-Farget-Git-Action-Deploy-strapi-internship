...
resource "aws_ecs_service" "strapi_service" {
  ...
  capacity_provider_strategy {
    capacity_provider = "FARGATE_SPOT"          # Task run on farget spot so task could stop anytime.
    weight            = 1
  }
...
#     weight            = 1   # Run at least some tasks on regular Fargate
#   }

#   capacity_provider_strategy {
#     capacity_provider = "FARGATE_SPOT"
#     weight            = 2   # Run more tasks on Spot to save cost
#   }

# It means there are total 3 task then it is use 2 task on farget spot and at least 1 task on farget .
